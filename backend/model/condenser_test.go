...
type CondenserTestInput struct {
...
func (setup *CondenserTestSetup) RunCondenserTests(t *testing.T, scenarios []CondenserTestScenario) {
	...
	for _, scenario := range scenarios {
		...
		t.Run(scenario.Name, func(t *testing.T) {
			...
			result, err := scenario.Input.Condenser.Condense(ctx, scenario.Input.Messages)
...
func TestTruncationCondenser(t *testing.T) {
	...
	setup.RunCondenserTests(t, []CondenserTestScenario{
		...
		{
			Name: "no eligible messages after preserving",
			Input: &CondenserTestInput{
				Condenser: NewTruncationCondenser(100000),
				Messages: []*Message{
					createTestMessage(MessageSourceUser, 0, 0, 0, 0),
					createTestMessage(MessageSourceUser, 0, 0, 0, 0),
					createTestMessage(MessageSourceUser, 0, 0, 0, 0),
					// Model message with high token usage
					createTestMessage(MessageSourceModel, 50000, 30000, 10000, 10000), // Total: 100000 > 80000
				},
			},
			Expected: CondenserTestExpectation{
		...
		{
			Name: "single eligible message",
			Input: &CondenserTestInput{
				Condenser: NewTruncationCondenser(100000),
				Messages: []*Message{
					createTestMessage(MessageSourceUser, 0, 0, 0, 0),                  // preserve
					createTestMessage(MessageSourceUser, 0, 0, 0, 0),                  // preserve
					createTestMessage(MessageSourceUser, 0, 0, 0, 0),                  // eligible (only one)
					createTestMessage(MessageSourceUser, 0, 0, 0, 0),                  // preserve
					createTestMessage(MessageSourceModel, 50000, 30000, 10000, 10000), // preserve
				},
			},
			Expected: CondenserTestExpectation{
				Result: &CondenserResult{}, // Only 5 messages, need 6 minimum for truncation
			},
		},
		{
			Name: "single eligible message with enough total messages",
			Input: &CondenserTestInput{
				Condenser: NewTruncationCondenser(100000),
				Messages: []*Message{
					createTestMessage(MessageSourceUser, 0, 0, 0, 0),                  // preserve
